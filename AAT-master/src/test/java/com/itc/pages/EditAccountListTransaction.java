...
public class EditAccountListTransaction extends BasePageObject {

	public EditAccountListTransaction(WebDriver driver) {
		super(driver);
	}
	...
	public static Logger logger = Logger.getLogger(EditAccountListTransaction.class);

	/* Web elements */

	By AccountingTab = By.xpath(".//*[@id='top-menu']//a[contains(.,'Accounting')]");
	By tranAccount = By.xpath(".//*[@id='top-menu']//a[contains(.,'List of Transactions')]");
	...
	public void clickOnChartAccount() throws Exception {
		try {
			...
			if (flag) {
				...
				WebElement acclink = driver.findElement(tranAccount);
	...
	public void clickOnAccountLink1() throws Exception {
		try {
			...
			By accountNameLink = By.xpath(
					"//table[contains(@class,'data-table margin-top')]//tr//a[contains(.,'" + accountName + "')]");
			logger.info("Account name" + accountName);

			if (isElementPresent(accountNameLink)) {
				setElement(accountNameLink).click();
				// Assert.assertTrue(isElementPresent(VendorNameLink), " Vendor
	...
	public void clickOnAccountLink2() throws Exception {
		try {
			...
			if (isElementPresent(accountNameLink)) {
				...
				System.out.println("Account Link is not displayed");
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public boolean isResultPageDisplayed() throws Exception {
		try {
			...
			if (flag1) {
				logger.info("Success page of Edit Account page is displayed");
